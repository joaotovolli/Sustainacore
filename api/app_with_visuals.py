...
def main(start_data, end_data):
    ...
    if company and company != "Select a Company":
        ...
        display_cols = ["DATE", "SourceCommonName", "URL"]
        st.dataframe(df_company[display_cols].head())

        # Radar chart (ESG Score comparison)
        ...
        # ESG over time line chart
        esg_keys = ["E_score", "S_score", "G_score"]
        score_cols = [f"{company.replace(' ', '_')}_diff"]
        esg_df = pd.concat([data[k][score_cols[0]].rename(k) for k in esg_keys], axis=1)
...
if __name__ == "__main__":
    ...
    if f"{start_data}_to_{end_data}" not in os.listdir("Data"):
        print(f"There isn't data for that range.")
