...
## VM2 setup steps
1) Install the preview Nginx config and reload:
```bash
sudo bash /opt/code/Sustainacore/tools/vm2/deploy_preview_nginx.sh
```

2) Validate locally before DNS:
```bash
curl -i -H 'Host: preview.sustainacore.org' http://127.0.0.1/
curl -i -H 'Host: preview.sustainacore.org' http://127.0.0.1/robots.txt
```
