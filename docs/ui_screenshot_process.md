...
- Waits for preview to serve the PR SHA (`build: <sha>` in page source).
...
The required PR gate is a CI-only compare that captures **home** only:
...
## Troubleshooting
- Preview not updated: confirm `deploy_preview_vm2` ran and preview source includes `build: <sha>`.
