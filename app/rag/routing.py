...
def _augment_full_mode(
    routing: str,
    hits: Sequence[Dict[str, Any]],
) -> Dict[str, Any]:
    extras: Dict[str, Any] = {}
    if routing == "smalltalk":
        extras.update({"type": "greeting", "suggestions": list(SMALLTALK_SUGGESTIONS)})
    elif routing == "no_hit":
        extras.update({
            "type": "no_hit",
    ...
    elif routing == "low_conf":
        extras.update({"type": "low_conf", "contexts": _build_source_payload(hits)})
    elif routing == "high_conf":
        extras.update({"type": "high_conf", "contexts": _build_source_payload(hits)})
    elif routing == "empty":
        extras.update({"type": "empty_query"})
    return extras
