        ...
        for item in sources:
            if isinstance(item, str):
                ...
                if isinstance(display, str):
                    ...
                    if candidate:
                        raw_sources.append(candidate)
        deduped_sources = _dedup_sources(raw_sources, settings.retriever_fact_cap)
        return {"answer": cleaned_answer, "sources": deduped_sources, "raw": response}
...
def _clean_answer_text(answer: str) -> str:
    ...
    for line in lines:
        ...
        if any(normalized.startswith(prefix) for prefix in banned_prefixes):
            continue
        if normalized.startswith("why this answer"):
            skip_debug = True
            continue
        if normalized.startswith("sources"):
            ...
            continue
        if skip_debug:
            # Skip until a blank line resets the section.
            continue
        cleaned.append(line)
    ...
    # Collapse multiple blank lines and ensure a neat paragraph.
    collapsed = re.sub(r"(\n\s*){2,}", "\n\n", "\n".join(cleaned)).strip()
