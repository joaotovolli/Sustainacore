...
_SMALL_TALK_PATTERN = re.compile(
    r"^(hi|hello|hey|hola|ol[áa]|oi|howdy|yo|thanks|thank you|thank you!|thanks!|good (morning|afternoon|evening))\b",
    re.IGNORECASE,
)


def _looks_like_small_talk(question: str) -> bool:
    """Return True when the input is a lightweight greeting/thanks."""

    text = (question or "").strip()
    if not text:
        return False
    if len(text) <= 40 and _SMALL_TALK_PATTERN.match(text):
        return True
    lowered = text.lower()
    if lowered in {"hi", "hello", "hey", "hola", "oi", "yo", "thanks", "thank you"}:
        return True
    return False
...
def run_pipeline(
    ...
    *,
    ...
    force_small_talk = _looks_like_small_talk(sanitized_q)
    ...
    if intent != "SMALL_TALK" and force_small_talk:
        intent = "SMALL_TALK"
        intent_info = {**intent_info, "forced_small_talk": True}
