...
const renderAsk2 = async (page) => {
  await page.waitForFunction(() => document.documentElement.dataset.ask2Ready === "true", { timeout: timeoutMs });
  ...
  await page.waitForSelector(".bubble--assistant:last-of-type .ask2-bubble", { timeout: timeoutMs });
};
...
const diffImages = (beforePath, afterPath, diffPath, reportPath) => {
  ...
  const mismatch = pixelmatch(before.data, after.data, diff.data, width, height, { threshold: 0.1 });
