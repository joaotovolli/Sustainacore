            ...
            SELECT job_id
             ...
             WHERE UPPER(TRIM(status)) IN ({status_clause})
               AND (
                   ...
                   OR (routine_code IS NULL AND UPPER(TRIM(routine_label)) = :routine_label)
                   OR UPPER(TRIM(routine_label)) = :routine_label
               )
...
def pick_job(conn, routine_code: str, routine_label: Optional[str]) -> Optional[JobRecord]:
    ...
    job_id = _select_job_id(cur, routine_code, routine_label, ["PENDING"])
    if not job_id:
        job_id = _select_job_id(cur, routine_code, routine_label, ["IN_PROGRESS"])
        if job_id:
...
def append_decision_notes(conn, approval_id: int, note: str) -> None:
    ...
    cur.execute(
        ...
        UPDATE proc_gemini_approvals
           SET decision_notes =
               CASE
                   WHEN decision_notes IS NULL THEN :note
                   ELSE decision_notes || CHR(10) || :note
               END
